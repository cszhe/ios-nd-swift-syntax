...
for r in oldMountainViewRent {
    newMountainViewRent.append(r * 1.2)
}
newMountainViewRent
...
for (u, p) in rapperPhoneNumbers {
    rapperPhoneNumbers[u] = p.replacingOccurrences(of: "415", with: "510")
}
rapperPhoneNumbers
...
switch firstLetter {
case _ where vowels.contains(firstLetter):
    newWord = word + "yay"
default:
    word.remove(at: word.startIndex)
    newWord = "\(word)\(firstLetter)ay"
